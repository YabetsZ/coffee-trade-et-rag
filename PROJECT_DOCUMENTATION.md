# Comprehensive Project Documentation: Amharic Coffee Trade RAG System
...
By leveraging state-of-the-art NLP models, this project provides an interactive question-answering terminal application that fetches accurate, context-grounded information from semantic searches over a localized text and PDF corpus.
...
*   **Reasoning:** We needed a model that natively maps Amharic concepts to dense vectors. The `multilingual-e5` family is state-of-the-art for non-English zero-shot retrieval. The `small` version was chosen specifically to allow high-throughput encoding on standard CPU hardware.
...
## 11. Conclusion
The Amharic Coffee Trade RAG System demonstrates that cutting-edge neural retrieval can be successfully localized. By stringently decoupling the factual database from the linguistic capabilities, we have constructed an assistant that answers complex Amharic queries while preventing hallucinations, utilizing pre-trained models rather than attempting the computationally impossible task of building an LLM from scratch.

---

## 12. References
1.  Vaswani, A. et al. (2017). "Attention Is All You Need." *Advances in Neural Information Processing Systems*.
2.  Xue, L. et al. (2020). "mT5: A massively multilingual pre-trained text-to-text transformer." *NAACL*.
